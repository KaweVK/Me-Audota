...
export const EditPet = () => {
  ...
  const handleSubmit = async (payload: PetFormSubmitPayload) => {
    ...
    setError(null)
    try {
      const updated = await updatePet(pet.id, { ...payload, anuncianteId: currentUser!.id })
      await refreshCurrentUser()
      navigate(`/pets/${updated.id}`, { replace: true })
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Não foi possível atualizar o pet agora.')
    } finally {
  ...
  if (!pet || !isOwner) return (
    <section className="w-full rounded-3xl border border-[var(--brand-line)] bg-white p-8">
      ...
      <Link to={pet ? `/pets/${pet.id}` : '/pets'} className="mt-6 inline-flex rounded-full bg-[var(--brand-highlight)] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--brand-highlight-strong)]">
        Voltar ao pet
      </Link>
    </section>
  )
